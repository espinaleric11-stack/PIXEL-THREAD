...
def guardar_datos():
    datos = {
        ...
        "logos": [
            {k: v for k, v in l.items() if k != "imagen_obj"}
...
def render_portal_cliente(nombre_cliente):
    ...
    # --- SECCIÓN DE SUBIR RECIBO CONVERTIDA EN BARRA DESPLEGABLE (EXPANDER) ---
    with st.expander("🧾 Subir Recibo de Pago"):
        ...
        if st.button("Enviar Comprabante", key=f"btn_enviar_recibo_{nombre_cliente}"):
            if recibo_subido:
